...
from webapp.forms import ProjectConfigForm
import pandas as pd
...
def create_project_bucket(cloud,user_id,project_id):
    ...
    logging.debug("Creating Project Bucket")
    ...
    try:
        ...
        with app.app_context():
            ...
            logging.debug("Project successfully initiated")
...
def validate_insert_train(user_id,project_id,folder_path,user_root,type):
    ...
    logging.debug("Validating Inserts")
    mongo=MongoDBmanagement(user_id=user_id,project_id=project_id)
    logger = App_Logger(user_id,project_id)
    ...
    try:
        ...
        with app.app_context():
            ...
            logging.debug("Project Training Files evaluated will be uploaded to cloud")
    except Exception as e:
        ...
        with app.app_context():
            ...
            logging.debug("Files couldnt be evaluated:")
        print(e)
        raise e
    try:
        ...
        with app.app_context():
            ...
            logging.debug("Project evaluated succesfully")
        ...
        with app.app_context():
            ...
            logging.debug(" Evaluated Training Files cannot be uploaded to cloud")
        print(e)
        raise e
...
def project_log_report(project_id):
    user_id = session['user_id']
    mongo = MongoDBmanagement(user_id,project_id)
    proj_logs = mongo.findRecordOnQuery('ProjectData','project_logs',{'project_id':project_id,'exception':False})
    if proj_logs:
        return render_template('project_log_report.html',logs=proj_logs)
    else:
        return render_template('project_log_report.html',logs=[])
...
def project_log_exception_report(project_id):
    ...
    print('--------------------sdfgh--------------')
    mongo = MongoDBmanagement(user_id,project_id)
    proj_exception_logs = mongo.findRecordOnQuery('ProjectData','project_exception_logs',{'project_id':project_id,'exception':'True'})
    print('--------------------sdfgh--------------')
    print([i for i in proj_exception_logs])
    if proj_exception_logs:
        return render_template('project_exception_log_report.html',details = proj_exception_logs)
    else:
        return render_template('project_exception_log_report.html',details = [])
...
def train_model(user,project):
    logging.debug("Started Train Model")
    user_name = user.get('user_name')
    ...
    try:
        ...
        if mltype == 'classification':
            basemodel = getclassification.BestClassificationModel(data,target)
            basemodel.preprocess_data()
            dump(basemodel.get_preprocess_pipe(),os.path.join(preprocess_folder,'preprocess_pipe.joblib'))
            model = basemodel.get_best_class_model() #fits and returns best model
            ...
            basemodel = getregression.BestRegessionModel(data,target)
            basemodel.preprocess_data()
            dump(basemodel.get_preprocess_pipe(),os.path.join(preprocess_folder,'preprocess_pipe.joblib'))
            model = basemodel.get_best_class_model()
            dump(model,os.path.join(preprocess_folder,'best-model.joblib'))
            dump(model,os.path.join(metrics,'best-model.joblib'))
    except Exception as e:
...
def train_project(project_id):
    ...
    if project_tag not in valid_tags:
        logging.debug("Cannot train model now, either files are not validated")
...
def test_model(user,project):  
    ...
    try:
        ...
        if mltype=='classification':
            ...
            test_data = model.inverse_preprocess_data(test_data)
...
def test_project(project_id):
    ...
    train_tag = project_detail.et('train_tag')
